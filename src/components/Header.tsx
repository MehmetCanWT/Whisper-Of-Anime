...
const Header = () => {
  ...
  }, [fetchSearchSuggestions]); // fetchSearchSuggestions'ı bağımlılıklara ekle
  ...
  const clearSearchStateAndSuggestions = () => {
    setSuggestions([]);
    setShowSuggestions(false);
    setSearchQuery('');
  };
  ...
  return (
    <header id="site-header" className={styles.siteHeader}>
      <Link href="/#" id="home-icon" aria-label="Go to homepage" className={styles.homeIcon} onClick={clearSearchStateAndSuggestions}>
          ...
          <div id="search-suggestions" className={styles.searchSuggestionsContainer}>
            ...
            {!isLoadingSuggestions && !searchError && suggestions.map((anime) => (
              <Link
                ...
                onClick={clearSearchStateAndSuggestions}
              >
                <Image
